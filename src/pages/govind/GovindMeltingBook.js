...
const GovindMeltingBook = () => {
  ...
  const [dataState, setDataState] = useState("valid");  const [rows, setRows] = useState([]);
  ...
  const handlePrintNow = useReactToPrint({
    ...
    documentTitle: 'Govind Melting Book - ' + dayjs().format("DD-MM-YYYY"),
    // onBeforeGetContent: () => {
  ...
  const deleteModal = async () => {
    ...
    const govindStockData = await fetchGovindStockList(1, Number.MAX_SAFE_INTEGER, token, dataState);
    const docs = govindStockData["data"];
    const count = govindStockData["count"];
    setTotalCount(count);
    ...
    selectedRowKeys.map((item, index) => {
      ...
      const matchedData = lossAcctData.find(row => row.transactionId === item && row.type === "Govind Melting")
      if (matchedData){
  ...
  return (
    <div>
      ...
      {!isPaginationEnabled && <div className="text-5xl text-center mb-8 print-only">Govind Melting Book</div>}
      ...
      <Table
        ...
        footer={isPaginationEnabled ? false : () => (
          <div className="print-footer">
            Govind Melting Book - {dayjs().format("DD-MMMM-YYYY")}
          </div>
        )}
        summary={() => {
          return (
