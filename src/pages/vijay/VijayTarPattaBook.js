...
const VijayTarPattaBook = () => {
  ...
  const handlePrintNow = useReactToPrint({
    ...
    documentTitle: 'Vijay Tarpatta Book - ' + dayjs().format("DD-MM-YYYY"),
    // onBeforeGetContent: () => {
  ...
  async function updateRows (dataType){
    ...
    const vijayStockData = await fetchVijayStockList(page, itemsPerPage, token, dataType);
    ...
    if (totalQty[0]["meltingReceive"].length === 0 || totalQty[0]["meltingReceive"][0]["meltingReceive"] === null){
        ...
        setMeltingWtBalance(totalQty[0]["meltingReceive"][0]["meltingReceive"].toFixed(2));      
    }
    ...
    if (totalQty[0]["tarpattaIssue"].length === 0 || totalQty[0]["tarpattaIssue"][0]["tarpattaIssue"] === null){
    ...
    if (totalQty[0]["tarpattaBhuka"].length === 0 || totalQty[0]["tarpattaBhuka"][0]["tarpattaBhuka"] === null){
    ...
    if (totalQty[0]["tarpattaLoss"].length === 0 || totalQty[0]["tarpattaLoss"][0]["tarpattaLoss"] === null){
  ...
  const handleSearch = async(selectedKeys, confirm, dataIndex, close) => {
    ...
    let allData = await fetchVijayStockList(1, Number.MAX_SAFE_INTEGER, token, dataState);
  ...
  const SelectAll = async() => {
    ...
    const vijayStockData = await fetchVijayStockList(1, Number.MAX_SAFE_INTEGER, token, "valid");
    const docs = vijayStockData["data"];
  ...
  return (
    <div>
      ...
      {!isPaginationEnabled && <div className="text-5xl text-center mb-8 print-only">Vijay Tarpatta Book</div>}
      ...
      <Table
        ...
        footer={isPaginationEnabled ? false : () => (
          <div className="print-footer">
            Vijay Tarpatta Book - {dayjs().format("DD-MMMM-YYYY")}
          </div>
        )}
        summary={() => {
          return (
